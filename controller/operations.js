...
export class Operations {
    ...
    sake(value) {
        ...
        account.bank.notifications.push(`Você realizou um saque no valor de ${value}.`);
        uptadeAccountStogare("bankAccounts", account);
    }
    ;
    lending(value, installMentsQuantity = 4) {
        const account = login.loginFind();
        if (!account)
            return;
        if (value <= 0)
        ...
        account.bank.balance += value;
        account.bank.notifications.push(`Você realizou um empréstimo de ${value}. Você dividiu em ${installMentsQuantity}x. Você pagará cada parcela no valor de ${valueInstallMents}.`);
        uptadeAccountStogare("bankAccounts", account);
    }
    ;
    deposit(value) {
        ...
        ;
        account.bank.balance += value;
        account.bank.notifications.push(`Você realizou um depósito de ${value}.`);
        uptadeAccountStogare("bankAccounts", account);
    }
    ;
    transfer(value, key) {
        ...
        if (value <= 0)
            return;
        const sameAccount = Object.values(account.bank.tranferKeys).find((transferKey) => key === transferKey);
        if (sameAccount)
            return console.error(`Não é possível transferir para si mesmo.`);
        ...
        if (!foundAccountDestination)
            return console.error(`Chave pix não encontrada.`);
        if (value > account.bank.balance)
            return console.error(`Você não possui saldo suficiente.`);
        account.bank.balance -= value;
        foundAccountDestination.bank.balance += value;
        account.bank.notifications.push(`Você realizou uma transferência no valor de ${value} para ${foundAccountDestination.name}.`);
        foundAccountDestination.bank.notifications.push(`Você recebeu uma transferência no valor de ${value} de ${account.name}`);
        uptadeAccountStogare("bankAccounts", account);
    ...
    seeLimit() {
        const account = login.loginFind();
        if (account)
            console.log(`${account.name}, seu limite: ${account.bank.limit}.`);
    }
    ;
    seeBalance() {
        const account = login.loginFind();
        if (account)
            console.log(`${account.name}, seu saldo: ${account.bank.balance}.`);
    }
    ;
}
;
