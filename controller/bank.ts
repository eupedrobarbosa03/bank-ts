...
export class Bank {
    ...
    accounts() {
       return getStorage<AccountDetails[]>("bankAccounts") || [];
    };

    accountCreate(name: string, cpf: string, password: string, email: string, telephone: string): void {
        const newAccount = new Account(name, cpf, password, email, telephone);
        if (!newAccount.create()) return;

        const existingAccount = this.accounts().find((account) => 
            account.cpf === cpf || account.email === email || account.telephone === telephone
        );
    ...
    accountUpdate(cpf: string, password: string, data: "email" | "telephone" | "password", newData: string) {

        const findAccount = this.accounts().find((account) => 
            account.cpf === cpf && account.password === password);

        if (!findAccount) return console.error(`Credenciais inválidas.`);

        function setterRegExp(setNewData: string, regExp: any) {
            if (!setNewData.match(regExp)) return false;
            return true;
        };

        const othersAccountsData = this.accounts().find((otherAccount) => 
            (data === "email" && newData === otherAccount.email) ||
            (data === "telephone" && newData === otherAccount.telephone)
        );

        if (othersAccountsData) return console.error(`E-mail e/ou telefone em uso.`);
        ...
        switch(data) {
            case "email":
                ...
                findAccount.bank.notifications.push(
                    `Seu e-mail foi alterado com sucesso. E-mail antigo: ${findAccount.email} | E-mail novo: ${newData}`
                );
            ...
            case "telephone":
                ...
                findAccount.bank.notifications.push(
                    `Seu número de telefone foi alterado com sucesso.`);
    ...
    accountDelete(cpf: string, password: string): void {
